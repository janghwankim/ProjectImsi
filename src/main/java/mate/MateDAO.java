...
//			String search = ten.getSearch();
			...
			try {
				con=pool.getConnection();
				//---------------------------------------------------------------------
		//		sql = "select b.* from (select rownum as rn, a.* from (select a.*, b.starttime, b.endtime, b.sleeptime, b.smoking, b.pet, b.sleepinghabbit, b.showertime, c.id from friend a inner join tendency b on a.id_no = b.id_no inner join member c on b.id_no=c.id_no where 1=1 order by  a.mate_no desc)a) b where (b.rn >? AND b.rn <=?)";
